...
class TestGetIP(unittest.TestCase):
    ...
    def test_get_ip_with_forwarded_header(self):
        ...
        response = self.app.get('/ip', headers={'X-Forwarded-For': '203.0.113.42'})
        self.assertEqual(response.status_code, 200)
        data = response.get_json()
    ...
    def test_get_ip_with_multiple_forwarded_ips(self):
        ...
        response = self.app.get('/ip', headers={'X-Forwarded-For': '198.51.100.1, 192.0.2.100'})
        self.assertEqual(response.status_code, 200)
        data = response.get_json()
    ...
    def test_get_ip_with_real_ip_header(self):
        ...
        response = self.app.get('/ip', headers={'X-Real-IP': '10.0.0.50'})
        self.assertEqual(response.status_code, 200)
        data = response.get_json()
